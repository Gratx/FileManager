...
import static co.ginx.filemanager.misc.IntentUtils.ACTION_OPEN_DOCUMENT_TREE;
import static co.ginx.filemanager.provider.ExternalStorageProvider.ROOT_ID_SECONDARY;
...
public class SAFManager {
    ...
    public static String getRootUri(Uri uri) {
        if (isTreeUri(uri)) {
            return co.ginx.filemanager.model.DocumentsContract.getTreeDocumentId(uri);
        }
        return co.ginx.filemanager.model.DocumentsContract.getDocumentId(uri);
    }

    private static Uri buildDocumentUriMaybeUsingTree(Uri uri, String docId) {
        return co.ginx.filemanager.model.DocumentsContract.buildDocumentUriMaybeUsingTree(uri, docId);
    }

    private static boolean isTreeUri(Uri uri) {
        return co.ginx.filemanager.model.DocumentsContract.isTreeUri(uri);
    }
